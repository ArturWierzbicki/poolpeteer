...
describe("options", () => {
    ...
    concurrencyTypes.forEach((concurrency) => {
        describe(`concurrency: ${concurrency}`, () => {
            ...
            test("retryDelay = 0", async () => {
                ...
                const ERROR_URL =
                    "http://example.com/we-are-never-visited-the-page";
            ...
            test("sameDomainDelay with one worker", async () => {
                ...
                const FIRST_URL =
                    "http://example.com/we-are-never-visiting-the-page";
                const SECOND_URL =
                    "http://another.tld/we-are-never-visiting-the-page";

                await cluster.task(
                    async ({ page, data: { url, counterShouldBe } }) => {
                        counter += 1;
            ...
            test("works with only functions", async () => {
                ...
                cluster.queue(
                    async ({ page, data }: { page: any; data: any }) => {
    ...
    test("other puppeteer objects like puppeteer-core", async () => {
        // expect.assertions(2);
        //
        // const executablePath = puppeteer.exe();
        // const cluster = await Cluster.launch<string>({

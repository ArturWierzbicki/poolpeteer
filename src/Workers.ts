...
export default class Workers<JobData = any, ReturnData = any> {
    ...
    async getWorker(
        ...
        debug(
            `Retrieving worker. State: ${JSON.stringify({
                workersStarting: this.workersStarting,
                workers: this.workers.length,
                workerIds: this.workers.map((w) => w.id).join(", "),
                lastLaunched: this.lastLaunchedWorkerTime,
            })}`
        );
